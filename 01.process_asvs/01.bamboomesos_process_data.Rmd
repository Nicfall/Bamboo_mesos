...
#saveRDS(seqtab,file="bamboomesos.seqtab.rds")
...
##where to save
setwd("~/Library/CloudStorage/GoogleDrive-nicolagk@hawaii.edu/My Drive/Summer2023_REU/Bamboo_mesos/01.process_asvs")
#write.csv(track,file="bamboomesos_readstats.csv",row.names=TRUE,quote=FALSE)
#saveRDS(seqtab.nochim,file="bamboomesos_seqtab.nochim.rds")
seqtab.nochim <- readRDS("bamboomesos_seqtab.nochim.rds")
```
...
```{r}
setwd("~/nicolagk@hawaii.edu - Google Drive/My Drive/Summer2023_REU/Bamboo_mesos/01.process_asvs")
...
## Remove ASVs supported by less than 0.001% of total reads

```{r}
sum(rowSums(seqtab.lulu)) #13546765
sum(rowSums(seqtab.lulu))*0.00001 #135

seqtab.lulu.keep2 <- seqtab.lulu.keep[colSums(seqtab.lulu.keep)>135]
tail(sort(colSums(seqtab.lulu.keep),decreasing=T)) #before
tail(sort(colSums(seqtab.lulu.keep2),decreasing=T)) #after
sum(rowSums(seqtab.lulu.keep2)) #13444749
##leaves 580 ASVs
```
...
samdf <- read.csv("../raw_data/bamboomesos_metadata.csv")
...
ps1 <- phyloseq(otu_table(seqtab.lulu.keep2, taxa_are_rows=FALSE), 
               sample_data(samdf), 
...
# otu_table()   OTU Table:         [ 580 taxa and 172 samples ]
# sample_data() Sample Data:       [ 172 samples by 14 sample variables ]
# tax_table()   Taxonomy Table:    [ 580 taxa by 9 taxonomic ranks ]
```
...
ps.mito #1 taxa to remove
ps.chlor <- subset_taxa(ps, Order=="Chloroplast")
ps.chlor #2 taxa to remove
...
ps.clean.decontam3 <- prune_taxa(taxa_sums(ps.clean.decontam2)!=0,ps.clean.decontam2)
ps.clean.decontam3
#any samples equal to 0? 
ps.clean.decontam <- prune_samples(sample_sums(ps.clean.decontam3)!=0,ps.clean.decontam3)
ps.clean.decontam 
# phyloseq-class experiment-level object
# otu_table()   OTU Table:         [ 554 taxa and 164 samples ]
# sample_data() Sample Data:       [ 164 samples by 16 sample variables ]
# tax_table()   Taxonomy Table:    [ 554 taxa by 9 taxonomic ranks ]
...
ggplot(data=df2, aes(x=Index, y=libsize))+
  ...
  ylim(120000,270000)+
  xlim(130,173)
#there's definitely a natural break between 132086 and 144974
#10% of that would be 13,208
...
ggplot(data=df2, aes(x=Index, y=libsize))+
  ...
  ylim(0,50000)+
  xlim(0,50)
##pretty obvious natural break ~47,000 but obviously I'm not going to sacrifice that many samples. It looks like another slight break ~11k
#11396 more specifically

set.seed(2939)

ps.rare <- rarefy_even_depth(ps.clean.decontam,sample.size=11396,replace=FALSE)
...
ps.rare #156 samples, 554 taxa

ps.clean.decontam

samdf.clean <- data.frame(ps.clean.decontam@sam_data)

samdf.low <- subset(samdf.clean,lib_size_clean<11000)
samdf.low
rownames(samdf.low)
...
ps.less <- phyloseq(otu_table(otu.less, taxa_are_rows=FALSE), 
...
##just negative control checks for curiosity
# ps.neg1 <- subset_samples(ps,Sample_type=="Negative")
# ps.neg <- prune_taxa(taxa_sums(ps.neg1)>0,ps.neg1)
# ps.neg
# taxa_sums(ps.neg)
# sample_sums(ps.neg)
# 
...
ps.exp <- subset_samples(ps.clean.decontam,Mesocosm_type=="Experiment")
ps.exp2 <- subset_samples(ps.exp,Exp_day!=20)
ps.exp.eco <- subset_samples(ps.exp2,Microbe_treatment=="EMO")
ps.exp.eco

ps_venn(ps.exp.eco,group="Sample_type",quantities=TRUE)
                       
                       ,main="ASVs per microbial stock",labels=c("LAB","ENV","MOS"),
