...
ps.clean1 <- readRDS("../01.process_asvs/bamboomesos_ps.lulu.clean.decontam.rds")
ps.clean1 #514 taxa, 169 samples
...
ps.clean <- subset_samples(ps.clean1,Exp_day!=20)
ps.clean
...
gg.bars.stocks.rich <- ggplot(df.div.stocks,aes(x=Microbe_treatment,y=Observed,fill=Microbe_treatment,label=Observed))+
  ...
  geom_text(nudge_y=4,color="black")+
  ...
  ylab("ASV richness")+
  guides(fill="none")+
  theme(axis.text.x=element_text(angle=45,hjust=1),plot.margin = margin(t=7.5,b = 50))+
  # geom_bar_pattern(
  #   pattern = "stripe",
  #   pattern_density = 0.2,     # How dense the pattern is (0–1)
  #   pattern_spacing = 0.2,    # Spacing between stripes
  #   pattern_color = "white",    # Color of the pattern
  #   pattern_angle = 45,        # Angle of stripes (degrees)
  #   colour = "black"           # Border color of the bar
  # )
...
```{r}
df.div.exp1 <- subset(df.all.div,Mesocosm_type=="Experiment")
#df.div.exp.no20 <- subset(df.div.exp,Exp_day!=20)
df.div.exp <- subset(df.div.exp1,Microbe_treatment!="AHK")

##desired order
df.div.exp$Microbe_treatment <- factor(df.div.exp$Microbe_treatment,levels=c("ECO","MMO","EMO","MEM","ALL"))

##calculate mean & standard error
df.div.exp.rich.se <- summarySE(df.div.exp,measurevar="Observed",groupvar=c("Microbe_treatment","Food_type","Sample_type","Mesocosm_type"))

##re-name variables for plotting
df.div.exp.rich.se$Food_type <- sub("Larval food","Standard diet",df.div.exp.rich.se$Food_type)
df.div.exp.rich.se$Food_type <- sub("Bamboo","Bamboo diet",df.div.exp.rich.se$Food_type)
df.div.exp.rich.se$Food_type <- factor(df.div.exp.rich.se$Food_type,levels=c("Standard diet","Bamboo diet"))

##current iteration = bar plot

gg.bars.exp.rich <- ggplot(df.div.exp.rich.se,aes(x=Microbe_treatment,y=Observed,fill=Microbe_treatment,alpha=Food_type))+
  ...
  ylim(0,65)+
  scale_alpha_manual(values=c(1,0.6),name="Diet",labels=c("Standard","Bamboo"))+
...
gg.richness.all <- ggarrange(gg.bars.stocks.rich,gg.bars.exp.rich,widths=c(0.3,1),labels=c("(a)","(b)"))
gg.richness.all
#ggsave("bamboomesos.rich.png",width=8,height=4)
```
...
gg.bars.stocks.simp <- ggplot(df.div.stocks,aes(x=Microbe_treatment,y=InvSimpson,fill=Microbe_treatment,label=invsimp.round))+
  ...
  ylab("Simpson's (inv.)")+
  guides(fill="none")+
  theme(axis.text.x=element_text(angle=45,hjust=1),plot.margin = margin(t=10,b = 50))+
  # geom_bar_pattern(
  #   pattern = "stripe",
  #   pattern_density = 0.2,     # How dense the pattern is (0–1)
  #   pattern_spacing = 0.2,    # Spacing between stripes
...
```{r}
##calculate mean & standard error
df.div.exp.simp.se <- summarySE(df.div.exp,measurevar="InvSimpson",groupvar=c("Microbe_treatment","Food_type","Sample_type","Mesocosm_type"))

##re-name variables for plotting
df.div.exp.simp.se$Food_type <- sub("Larval food","Standard diet",df.div.exp.simp.se$Food_type)
df.div.exp.simp.se$Food_type <- sub("Bamboo","Bamboo diet",df.div.exp.simp.se$Food_type)
df.div.exp.simp.se$Food_type <- factor(df.div.exp.simp.se$Food_type,levels=c("Standard diet","Bamboo diet"))

##current iteration = bar plot

gg.bars.exp.simp <- ggplot(df.div.exp.simp.se,aes(x=Microbe_treatment,y=InvSimpson,fill=Microbe_treatment,alpha=Food_type))+
  #geom_point(data=df.all.div.nocon.noday20.noahk,alpha=0.5,position=position_jitterdodge(0.9))+
  ggtitle("Experiment samples")+
  geom_bar(stat="identity",position="dodge",color="black")+
...
#gg.bars.stocks.simp + gg.bars.exp.simp

gg.simpness.all <- ggarrange(gg.bars.stocks.simp,gg.bars.exp.simp,widths=c(0.3,1),labels=c("(a)","(b)"))
gg.simpness.all
#ggsave("bamboomesos.simp.png",width=8,height=4)
```
...
```{r}
df.div.lar <- subset(df.div.exp,Sample_type=="Larvae")

glm.obs.lar.nor <- glmmTMB(Observed~Microbe_treatment*Food_type+offset(log(lib_size_clean)),data=df.div.lar,dispformula=~Microbe_treatment)
summary(glm.obs.lar.nor)
glm.obs.lar.poi <- glmmTMB(Observed~Microbe_treatment*Food_type+offset(log(lib_size_clean)),data=df.div.lar,family=poisson,dispformula=~Microbe_treatment)
glm.obs.lar.com <- glmmTMB(Observed~Microbe_treatment*Food_type+offset(log(lib_size_clean)),data=df.div.lar,family=compois,dispformula=~Microbe_treatment)
#summary(glm.obs.lar.com)
glm.obs.lar.nb1 <- glmmTMB(Observed~Microbe_treatment*Food_type+offset(log(lib_size_clean)),data=df.div.lar,family=nbinom1,dispformula=~Microbe_treatment)
glm.obs.lar.nb2 <- glmmTMB(Observed~Microbe_treatment*Food_type+offset(log(lib_size_clean)),data=df.div.lar,family=nbinom2,dispformula=~Microbe_treatment)

AICtab(glm.obs.lar.nor,glm.obs.lar.poi,glm.obs.lar.com,glm.obs.lar.nb1,glm.obs.lar.nb2)

summary(glm.obs.lar.nor)

glm.obs.lar.nor.noint <- update(glm.obs.lar.nor,.~.-(Microbe_treatment:Food_type))
summary(glm.obs.lar.nor.noint)

anova(glm.obs.lar.nor,glm.obs.lar.nor.noint) #with interaction is better
##0.003588 **

glm.obs.lar.nor.resid <- simulateResiduals(fittedModel = glm.obs.lar.nor, plot = T)
#qqnorm(resid(glm.obs.lar.nor))
#qqline(resid(glm.obs.lar.nor))

glm.obs.lar.nor.em <- emmeans(glm.obs.lar.nor,~Microbe_treatment:Food_type)

multcomp::cld(glm.obs.lar.nor.em)
```

### Water

```{r}
df.div.wat <- subset(df.div.exp,Sample_type=="Water")

glm.obs.wat.nor <- glmmTMB(Observed~Microbe_treatment*Food_type+offset(log(lib_size_clean)),data=df.div.wat,dispformula=~Microbe_treatment)
summary(glm.obs.wat.nor)
glm.obs.wat.poi <- glmmTMB(Observed~Microbe_treatment*Food_type+offset(log(lib_size_clean)),data=df.div.wat,family=poisson,dispformula=~Microbe_treatment)
glm.obs.wat.com <- glmmTMB(Observed~Microbe_treatment*Food_type+offset(log(lib_size_clean)),data=df.div.wat,family=compois,dispformula=~Microbe_treatment)
#summary(glm.obs.wat.com)
glm.obs.wat.nb1 <- glmmTMB(Observed~Microbe_treatment*Food_type+offset(log(lib_size_clean)),data=df.div.wat,family=nbinom1,dispformula=~Microbe_treatment)
#summary(glm.obs.wat.nb1)
glm.obs.wat.nb2 <- glmmTMB(Observed~Microbe_treatment*Food_type+offset(log(lib_size_clean)),data=df.div.wat,family=nbinom2,dispformula=~Microbe_treatment)

AICtab(glm.obs.wat.nor,glm.obs.wat.poi,glm.obs.wat.com,glm.obs.wat.nb1,glm.obs.wat.nb2)
#summary(glm.obs.wat.nor)

glm.obs.wat.nor.noint <- update(glm.obs.wat.nor,.~.-(Microbe_treatment:Food_type))
summary(glm.obs.wat.nor.noint)

anova(glm.obs.wat.nor,glm.obs.wat.nor.noint) #with interaction is better
##2.2e-16 ***
```
...
```{r}
glm.simp.lar.nor <- glmmTMB(InvSimpson~Microbe_treatment*Food_type,data=df.div.lar)
summary(glm.simp.lar.nor)

#AICtab(glm.simp.lar.nor,glm.simp.lar.poi,glm.simp.lar.com,glm.simp.lar.nb1,glm.simp.lar.nb2)

glm.simp.lar.nor.noint <- update(glm.simp.lar.nor,.~.-(Microbe_treatment:Food_type))
summary(glm.simp.lar.nor.noint)

anova(glm.simp.lar.nor,glm.simp.lar.nor.noint) #with interaction is better
##0.00761 **

glm.simp.lar.nor.resid <- simulateResiduals(fittedModel = glm.simp.lar.nor, plot = T)
#qqnorm(resid(glm.simp.lar.nor))
#qqline(resid(glm.simp.lar.nor))

glm.simp.lar.nor.em <- emmeans(glm.simp.lar.nor,~Microbe_treatment:Food_type)

multcomp::cld(glm.simp.lar.nor.em)
```

### Water

```{r}
glm.simp.wat.nor <- glmmTMB(InvSimpson~Microbe_treatment*Food_type,data=df.div.wat)
summary(glm.simp.wat.nor)

#AICtab(glm.simp.wat.nor,glm.simp.wat.poi,glm.simp.wat.com,glm.simp.wat.nb1,glm.simp.wat.nb2)

glm.simp.wat.nor.noint <- update(glm.simp.wat.nor,.~.-(Microbe_treatment:Food_type))
summary(glm.simp.wat.nor.noint)

anova(glm.simp.wat.nor,glm.simp.wat.nor.noint) #with interaction is better
##0.01795 *

glm.simp.wat.nor.resid <- simulateResiduals(fittedModel = glm.simp.wat.nor, plot = T)
#qqnorm(resid(glm.simp.wat.nor))
#qqline(resid(glm.simp.wat.nor))

glm.simp.wat.nor.em <- emmeans(glm.simp.wat.nor,~Microbe_treatment:Food_type)

multcomp::cld(glm.simp.wat.nor.em)
```
