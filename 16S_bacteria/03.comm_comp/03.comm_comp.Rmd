---
title: "Bamboo mesos community composition"
...
## Phylum ##
ps.stocks.rel.phy <- tax_glom(ps.stocks.rel, taxrank = "Phylum") 
...
```{r}
## Class ##
ps.stocks.rel.cla <- tax_glom(ps.stocks.rel, taxrank = "Class") 
ps.stocks.rel.cla #12

# Calculate taxa sum
top5 = head(sort(colSums(otu_table(ps.stocks.rel.cla)), decreasing = TRUE), 10)
# Combine count and taxonomyTable
top5 = cbind(as.data.frame(tax_table(ps.stocks.rel.cla)[names(top5),]), Count = top5)
top5
#Gammaproteobacteria
#Alphaproteobacteria
#Bacilli
...
```{r}
mdf.stocks <- prep_mdf(ps.stocks.rel,subgroup_level="Family")

# Create a color object for the specified data
col.mdf.stocks <- create_color_dfs(mdf.stocks, top_orientation = F,group_level="Class",subgroup_level="Family",selected_groups=c("Gammaproteobacteria","Alphaproteobacteria","Bacilli","Negativicutes"),cvd=TRUE)
